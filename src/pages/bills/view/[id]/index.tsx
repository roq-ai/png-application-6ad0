...
import { getBillById } from 'apiSdk/bills';
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('bill', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/bills/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Bill Date"
                  text={data?.bill_date ? format(parseISO(data?.bill_date as unknown as string), 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Bill Amount" text={data?.bill_amount} />

                <FormListItem label="Bill Paid" text={data?.bill_paid} />
                ...
                {hasAccess('meter_reading', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Meter Reading"
                    text={
                      <Link as={NextLink} href={`/meter-readings/view/${data?.meter_reading?.id}`}>
                        {data?.meter_reading?.reading_date}
                      </Link>
                    }
                  />
                )}
