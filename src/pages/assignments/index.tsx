  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.assignment_date ? format(parseISO(record?.assignment_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_assignment_account_manager_idTouser',
          header: 'User Assignment Account Manager Id Touser',
          accessorKey: 'user_assignment_account_manager_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_assignment_account_manager_idTouser?.id}`}
            >
              {record.user_assignment_account_manager_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_assignment_meter_reader_idTouser',
          header: 'User Assignment Meter Reader Id Touser',
          accessorKey: 'user_assignment_meter_reader_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_assignment_meter_reader_idTouser?.id}`}
            >
              {record.user_assignment_meter_reader_idTouser?.email}
            </Link>
          ),
        }
      : null,
