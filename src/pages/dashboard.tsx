...
import { MeterReadingListPage as ShowCaseList } from 'pages/meter-readings';

function DashboardPage() {
  const { session } = useSession();

  const meterReadingStat = useSWR<PaginatedInterface<MeterReadingInterface>>(
    () => `/meter-readings`,
    () => getMeterReadings({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: meterReadingStat?.data?.totalCount,
      isLoading: meterReadingStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`meter-readings`} passHref legacyBehavior>
                  <Button
