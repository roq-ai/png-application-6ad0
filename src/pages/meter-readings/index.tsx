...
type ColumnType = ColumnDef<MeterReadingInterface, unknown>;

interface MeterReadingListPageProps {
...
export function MeterReadingListPage(props: MeterReadingListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MeterReadingInterface>>(
    () => `/meter-readings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MeterReadingInterface) => {
    if (hasAccess('meter_reading', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/meter-readings/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.reading_date ? format(parseISO(record?.reading_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'reading_value', header: 'reading_value', accessorKey: 'reading_value' },
    { id: 'bill_calculated', header: 'Bill Calculated', accessorKey: 'bill_calculated' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/meter-readings/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="meter_reading" />
        </Flex>
